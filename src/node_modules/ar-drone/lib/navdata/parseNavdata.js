...
var exports = module.exports = function parseNavdata(buffer) {
  ...
  while (true) {
    ...
    // If option is unknown (not present in OPTION_IDS), ignore it safely
    if (!optionName) {
      // store minimal debug info and continue parsing next option
      try {
        navdata['unknown_0x' + optionId.toString(16)] = { size: length - 4 };
      } catch (e) {
        // noop
      }
      continue;
    ...
    if (exports.OPTION_PARSERS[optionName]) {
      navdata[optionName] = exports.OPTION_PARSERS[optionName](optionReader);
    } else {
      // option exists in OPTION_IDS but no parser implemented yet -> store placeholder
      navdata[optionName] = { _notImplemented: true, optionId: optionId };
    }
  }
